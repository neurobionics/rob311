...
class BBController(Controller):
    ...
    """
    def __init__(self, theta_roll_pid, theta_pitch_pid, phi_roll_pid, phi_pitch_pid, **kwargs):
        Controller.__init__(self, **kwargs)
        self.MAX_TZ = 0.5 # Nm
        self.MAX_VELOCITY = 1.0 # rad/sec

        self.DELTA_THETA_KP = 0.1
        self.DELTA_THETA_KD = 0.01

        self.DELTA_PHI_KP = 0.01
        self.DELTA_PHI_KD = 0.001
        ...
        self.theta_roll_pid = theta_roll_pid
        self.theta_pitch_pid = theta_pitch_pid

        self.phi_roll_pid = phi_roll_pid
        self.phi_pitch_pid = phi_pitch_pid

        self.michigan_lights()

    def michigan_lights(self):
        self.dots = init_lights(MAX_BRIGHTNESS)
        random_colors = np.array([random_color(), random_color(), random_color()])

        colored_dots = np.random.randint(3, size=N_DOTS)
        self.dots[:] = random_colors[colored_dots][:]
        self.dots.show()  

    # LEFT THUMBSTICK: Z-AXIS ROTATION CW and CCW
    def on_L3_up(self, value):
        # Inverting y-axis value
        self.Tz = -1.0 * self.MAX_TZ * value/JOYSTICK_SCALE
        print(self.Tz)
    ...
    # RIGHT THUMBSTICK: Y-AXIS UP/DOWN and X-AXIS RIGHT/LEFT
    def on_R3_up(self, value):
        # Inverting y-axis value
        self.dphi_x_sp = -1.0 * self.MAX_VELOCITY * value/JOYSTICK_SCALE
        print(self.dphi_x_sp)
    ...
    def on_R3_right(self, value):
        # Inverting y-axis value
        self.dphi_y_sp = 1.0 * self.MAX_VELOCITY * value/JOYSTICK_SCALE
        print(self.dphi_y_sp)
    ...
    def on_up_arrow_press(self):
        self.phi_kp += self.DELTA_PHI_KP

        if self.phi_kp > MAX_PHI_KP:
            self.phi_kp = MAX_PHI_KP
            print("Maxed out Phi Kp at {:.2f}.".format(self.phi_kp))
        else:
            print("Increased Phi Kp to {:.2f}.".format(self.phi_kp))

        self.phi_roll_pid.Kp = self.phi_kp
        self.phi_pitch_pid.Kp = self.phi_kp    
    ...
    def on_down_arrow_press(self):
        ...
        if self.phi_kp < MIN_PHI_KP:
            self.phi_kp = MIN_PHI_KP
            print("Bottomed out Phi Kp at {:.2f}.".format(self.phi_kp))
        else:
            print("Decreased Phi Kp to {:.2f}".format(self.phi_kp))
    ...
    def on_right_arrow_press(self):
        self.phi_kd += self.DELTA_PHI_KD
        if self.phi_kd > MAX_PHI_KD:
            self.phi_kd = MAX_PHI_KD
            print("Maxed out Phi Kd at {:.2f}.".format(self.phi_kd))
        else:        
            print("Increased Phi Kd to {:.2f}.".format(self.phi_kd))

        self.phi_roll_pid.Kd = self.phi_kd
        self.phi_pitch_pid.Kd = self.phi_kd        

    def on_left_arrow_press(self):
        self.phi_kd -= self.DELTA_PHI_KD
        if self.phi_kd < MIN_PHI_KD:
            self.phi_kd = MIN_PHI_KD
            print("Bottomed out Phi Kd at {:.2f}.".format(self.phi_kd))
        else:        
            print("Decreased Phi Kd to {:.2f}".format(self.phi_kd))
    ...
    def on_triangle_press(self):
        self.theta_kp += self.DELTA_THETA_KP
        ...
        self.theta_roll_pid.Kp = self.theta_kp
        self.theta_pitch_pid.Kp = self.theta_kp
    ...
    def on_circle_press(self):
        self.theta_kd += self.DELTA_THETA_KD
        ...
        self.theta_roll_pid.Kd = self.theta_kd
        self.theta_pitch_pid.Kd = self.theta_kd
...
N_DOTS = 83
MAX_BRIGHTNESS = 0.065

COLORS = np.array([(53, 118, 174), (240, 100, 0)])
N_COLORS = len(COLORS)

def random_color():
    return (random.randrange(0, 7) * 32, \
        random.randrange(0, 7) * 32, \
            random.randrange(0, 7) * 32)
...
if __name__ == "__main__":
    ...
    bb_controller = BBController(theta_roll_pid, theta_pitch_pid, \
                phi_roll_pid, phi_pitch_pid, \
